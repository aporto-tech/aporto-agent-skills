...
concept:
  id: revops
  name: Revops
tags: [marketingskills, marketing, revops, lifecycle, paid-acquisition, sales]
when_to_use:
  - "When the user wants help with revenue operations, lead lifecycle management, or marketing-to-sales handoff processes.
  - Also use when the user mentions 'RevOps,' 'revenue operations,' 'lead scoring,' 'lead routing,' 'MQL,' 'SQL,' 'pipeline stages,' 'deal desk,' 'CRM automation,' 'marketing-to-sales handoff,' 'data hygiene,' 'leads aren't getting to sales,' 'pipeline management,' 'lead qualification,' or 'when should marketing hand off to sales.' Use this for anything involving the systems and processes that connect marketing to revenue.
  - For cold outreach emails, see cold-email.
  - For email drip campaigns, see emails.
...
Use this Aporto agent skill when the user needs the Revops workflow from the Marketing Skills catalog.

## Source Snapshot

This Aporto skill is adapted from Corey Haines' Marketing Skills repository at `skills/revops/SKILL.md`. Use it as an instruction layer. If real execution is needed, call an Aporto capability through the routing API.
