...
concept:
  id: aso
  name: ASO
tags: [marketingskills, marketing, aso]
when_to_use:
  - "When the user wants to audit or optimize an App Store or Google Play listing.
  - Also use when the user mentions 'ASO audit,' 'app store optimization,' 'optimize my app listing,' 'improve app visibility,' 'app store ranking,' 'audit my listing,' 'why aren't people downloading my app,' 'improve my app conversion,' 'keyword optimization for app,' or 'compare my app to competitors.' Use when the user shares an App Store or Google Play URL and wants to improve it."
...
Use this Aporto agent skill when the user needs the ASO workflow from the Marketing Skills catalog.

## Source Snapshot

This Aporto skill is adapted from Corey Haines' Marketing Skills repository at `skills/aso/SKILL.md`. Use it as an instruction layer. If real execution is needed, call an Aporto capability through the routing API.
